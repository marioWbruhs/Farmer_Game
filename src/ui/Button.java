...
public class Button {
    ...
    public Button(float x, float y, float w, float h, int[] color, String text)
    {
        ...
        this.buttonController = new Box(x, y, w, h, color, text);
    ...
    public void draw(int[] screenSize)
    {
        ...
        if(this.id == "PLAY GAME") {
            fontRenderer.drawString(this.getId(), (float) ((this.x * (screenSize[0] / 2) + this.w * (screenSize[0] / 2.1)) - (fontRenderer.getWidth(this.getId()) * 2)), -15, 0xff000000);
        }
        else if(this.id == "QUIT GAME")
        {
            fontRenderer.drawString(this.getId(), (float) ((this.x * (screenSize[0] / 2 ) + this.w * (screenSize[0] / 2.1)) - (fontRenderer.getWidth(this.getId()) * 2)), 190, 0xff000000);
        }
        else if(this.id == "MAP EDITOR")
        {
            fontRenderer.drawString(this.getId(), (float) ((this.x * (screenSize[0] / 2 ) + this.w * (screenSize[0] / 2.025)) - (fontRenderer.getWidth(this.getId()) * 2)), 75, 0xff000000);
        }
